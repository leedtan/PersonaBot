...
while True:
    user = input('Select the user you want to talk to:')
    chatbot_user_index = dataset.index_user(user)
    if chatbot_user_index == dataset.unknown_user_index:
        print("Sorry, we don't know how he/she would speak :P")
    else:
        break

while True:
    user = input("Select the user you want to pretend as (or leave it alone if you don't care):")
    if user == '':
        speaker_index = dataset.unknown_user_index
        break
    speaker_index = dataset.index_user(user)
    if speaker_index == dataset.unknown_user_index:
        print("Sorry, we don't know how we should behave :P")
    else:
        break
...
while True:
    msg = input()
    speaker_history.append(speaker_index)
    addressee_history.append(chatbot_user_index)
    words = word_tokenize(msg.lower())
    words = words[:args.max_sentence_length_allowed - 2]
    words.insert(0, START)
    words.append(EOS)
    words = [dataset.index_word(w) for w in words]
    sentence_history.append(words)

    speaker_history.append(chatbot_user_index)
    addressee_history.append(speaker_index)
    sentence_history.append([0] * 50)

    turns, sentence_lengths_padded, speaker_padded, \
        addressee_padded, words_padded, words_reverse_padded = \
        collate_as_tensor([(addressee_history, speaker_history, sentence_history)])
